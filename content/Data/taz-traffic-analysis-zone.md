...
与网格/H3/地块等四种划分策略的关系见 [[spatial_units_report|四种空间单元划分策略对比]]——TAZ 属于其中的「语义边界对齐较高、数据挂接较难」一端，且是唯一以「出行 OD 建模」为原生目的的分区体系。TAZ 服务的模型内核是 [[four-step-model|四阶段模型]]；其在项目尺度上的应用见 [[traffic-impact-assessment|交通影响评估（TIA）]]。
