---
title: 命名实体识别
description: 命名实体识别的核心概念、方法、项目与相关研究笔记。
tags:
  - NLP
  - NER
...
> [!note] 合并说明
> 本页是 NER 的权威入口，已合并原 `NER.md` 中关于 LLM 生成示例、知识蒸馏
> 和小模型微调的个人判断。具体模型规模与效果必须以实际数据集和实验为准。
...
LLM 不一定直接承担生产环境中的全部 NER 推理。一个可测试的工程路线是：

1. 用人工定义的实体类型和少量高质量样例约束 LLM；
2. 生成候选训练样本并由人工抽检、去重和纠错；
3. 用接受后的样本微调较小的序列标注或生成模型；
4. 在独立测试集上比较准确率、召回率、延迟和成本。

这里的 LLM 输出属于候选标注而不是自动真值；如果没有人工复核和独立测试，
合成数据可能把模型偏差继续传递给小模型。
