---
title: AI 的分类
description: AI 的分类的核心概念、方法、项目与相关研究笔记。
tags:
  - AI
  - GenAI
  - 通识
...
- **强化学习**：通过试错与环境交互优化策略（如游戏AI、机器人控制）。站内入口见[[../RL/index|强化学习]]。
- **生成式模型**：基于概率生成新数据（如GPT生成文本、Stable Diffusion生成图像）。站内入口见[[GenAI/index|生成式 AI]]。

#### 2. **技术分支**
- **自然语言处理（NLP）**：文本理解与生成（如机器翻译、聊天机器人）。站内示例见[[../DL/NER|命名实体识别]]。
...
- **深度学习**：CNN（图像）、RNN（序列数据）、Transformer（多模态）。学习入口见[[../DL/index|深度学习]]。
- **大模型**：参数量超大规模的预训练模型（如GPT、通义千问）。站内入口见[[LLM/index|大语言模型]]。
